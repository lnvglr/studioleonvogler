 ...
 * them directly as block files to public/codepoint-names/ (no intermediate JSON).
 * Names are stored as final display strings: space-separated uppercase (e.g.
 * "PLUS SIGN", "LATIN CAPITAL LETTER A", "UPWARDS ARROW") so no runtime formatting
 * is needed. Run: node scripts/generate-glyph-map.js
 */
import fs from "fs";
...
const CODEPOINT_NAMES_DIR = path.join(__dirname, "..", "public", "codepoint-names");
...
function generateName(name, codepoint) {
  ...
  // Arabic-Indic digits (U+0660–U+0669) and Extended Arabic-Indic / Farsi–Urdu (U+06F0–U+06F9): use explicit names
  if (cp >= 0x0660 && cp <= 0x0669) {
    const digit = cp - 0x0660;
    return `ARABIC-INDIC DIGIT ${DIGIT_WORDS[digit]}`;
  }
  if (cp >= 0x06f0 && cp <= 0x06f9) {
    const digit = cp - 0x06f0;
    return `EXTENDED ARABIC-INDIC DIGIT ${DIGIT_WORDS[digit]}`;
  }

  // Superscript figures ⁰¹²³⁴⁵⁶⁷⁸⁹: DIGIT ZERO SUPERSCRIPT, ...
  const superscriptDigit = cp === 0x2070 ? 0 : cp === 0x00b9 ? 1 : cp === 0x00b2 ? 2 : cp === 0x00b3 ? 3 : (cp >= 0x2074 && cp <= 0x2079) ? cp - 0x2074 + 4 : -1;
  if (superscriptDigit >= 0) {
    return `DIGIT ${DIGIT_WORDS[superscriptDigit]} SUPERSCRIPT`;
  }

  // Subscript figures ₀₁₂₃₄₅₆₇₈₉: DIGIT ZERO SUBSCRIPT, ...
  if (cp >= 0x2080 && cp <= 0x2089) {
    const digit = cp - 0x2080;
    return `DIGIT ${DIGIT_WORDS[digit]} SUBSCRIPT`;
  }

  // Circled digits ①–⑨ (U+2460–U+2469), ⓪ (U+24EA): DIGIT {literal} WITH CIRCLE
  if (cp >= 0x2460 && cp <= 0x2469) {
    const digit = cp - 0x2460 + 1; // 2460 = ONE, ..., 2469 = NINE
    return `DIGIT ${DIGIT_WORDS[digit]} WITH CIRCLE`;
  }
  if (cp === 0x24ea) {
    return `DIGIT ZERO WITH CIRCLE`;
  }

  // Get the actual character (cp stays in scope for rest of function)
    ...
    if (addedSignType && type === "sign") {
      ...
      if (typeIndex !== -1) {
        if (typeIndex < parts.length - 1) {
          ...
          // Type is at the end, everything before it is literal (excluding script words when script is set)
  ...
  // Build the name
  const nameParts = [];
  // Arabic-Indic digits (U+0660–U+0669) → ARABIC-INDIC DIGIT ZERO; Eastern (U+06F0–U+06F9) → FARSI URDU DIGIT ZERO
  const digitPrefix =
    type === "digit"
      ? cp >= 0x0660 && cp <= 0x0669
        ? "Arabic-Indic"
  ...
  if (digitPrefix) {
    ...
    nameParts.push(script === "nko" ? "NKO" : script);
  }
  ...
  // e.g., "MULTIPLICATION SIGN" not "SIGN MULTIPLICATION"
  // e.g., "LEFT SQUARE BRACKET" / "RIGHT CURLY BRACKET" so left/right is preserved
  const literalBeforeType = (type === "sign" || type === "parenthesis" || type === "arrow" || type === "bracket") && !script;
  // For script names with type at end (e.g. NKO COMBINING SHORT HIGH TONE, NKO COMBINING NASALIZATION MARK), put literal before type
...
for (const line of lines) {
  ...
  // Validate that the generated character has the correct codepoint
  if (actualCodepoint !== expectedCodepoint) {
    ...
    continue; // Skip this entry if there's a mismatch
  }
...
if (errors.length > 0) {
  ...
  errors.slice(0, 20).forEach(err => {
    console.error(`  ${err.hex}: ${err.name} - expected ${err.expected} (0x${err.expected.toString(16)}), got ${err.actual} (0x${err.actual.toString(16)})`);
  });
  if (errors.length > 20) {
...
// Split by Unicode block (high byte) and write block files directly — no intermediate JSON
const byBlock = {};
...
fs.mkdirSync(CODEPOINT_NAMES_DIR, { recursive: true });
for (const [blockId, blockData] of Object.entries(byBlock)) {
  const outPath = path.join(CODEPOINT_NAMES_DIR, `${blockId}.json`);
  fs.writeFileSync(outPath, JSON.stringify(blockData), "utf8");
  console.log(`Wrote ${blockId}.json (${Object.keys(blockData).length} entries)`);
}

console.log(`✔ Generated ${Object.keys(out).length} readable glyph names → ${Object.keys(byBlock).length} block files in public/codepoint-names/`);
