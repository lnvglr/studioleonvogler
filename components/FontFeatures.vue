<template>
  <section
    v-if="
      props.font.id === 'diode-global-next' || props.font.id === 'seoul-grotesk'
    "
    id="features"
  ...
  >
    <div class="px-5 sm:px-10 md:px-20 py-8 md:py-12">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8 mb-8">
        <div class="md:col-span-8">
          ...
          <p class="text-neutral-800 leading-relaxed">
            {{ font.name }} comes with many OpenType features which can be used
            to tailor functionality and aesthetics to your specific needs. Some
            of these features can be combined to form a great number of
            alternative variations.
          </p>
        </div>
      </div>

      <hr class="hidden md:block border-t border-neutral-800 my-8" />

      <!-- Feature Table Header -->
      <div
        class="hidden md:grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-10 mb-8"
      >
        ...
        >
          ...
          <div class="md:col-span-2">
            <div class="flex items-start gap-2 mb-2 flex-wrap">
              <span
                class="inline-block px-2 py-1 bg-black/5 text-black text-xs font-mono"
              >
                {{ feature.tag }}
              </span>
            ...
            <p
              ...
              class="text-sm text-neutral-800 mt-2 leading-relaxed"
            >
              {{ feature.description }}
            </p>
          </div>
          ...
          <div
            data-off-column
            class="md:col-span-5 !leading-tight text-neutral-900 text-3xl md:text-4xl lg:text-5xl break-words"
            :style="{
            ...
            v-html="getProcessedExampleText(feature)"
          />
          ...
          <div
            ...
            v-html="feature.enabledText || getProcessedExampleText(feature)"
          />
      ...
      <div v-else class="text-neutral-800 py-8">No features found.</div>
    </div>
  </section>
</template>
...
const extractFeaturesFromFont = async () => {
  ...
  if (
    props.font.id !== "diode-global-next" &&
    props.font.id !== "seoul-grotesk"
  ) {
    features.value = [];
  ...
  try {
    ...
    if (!samsaFont) {
      console.error(
        "[FontFeatures] Failed to load font with Samsa - loadFont returned null"
      );
      features.value = [];
      isLoading.value = false;
      return;
    ...
    if (!gsubFeatures) {
      console.error(
        "[FontFeatures] No GSUB features found. Full font object:",
        {
    ...
    console.error(
      "[FontFeatures] Failed to extract features from font:",
      error
    );
    console.error(
      "[FontFeatures] Error stack:",
...
const getProcessedExampleText = (feature: FontFeature): string => {
  ...
  // If highlighted is an array, use those strings directly
  // If it's a string, split into individual characters
  const highlightedStrings = Array.isArray(metadata.highlighted)
  ...
  const processedParts = parts.map((part) => {
    ...
    // Find all matches of highlighted strings in order
    const matches: Array<{ start: number; end: number; text: string }> = [];

    highlightedStrings.forEach((highlightedStr) => {
      const escaped = highlightedStr.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    ...
    for (const match of matches) {
      if (mergedMatches.length === 0) {
        ...
        if (match.start <= last.end) {
          ...
          // Not overlapping, add new match
          mergedMatches.push({ start: match.start, end: match.end });
        }
      }
    }

    // Build result: dim everything except the matched ranges
    for (const match of mergedMatches) {
      // Add dimmed text before this match
      if (match.start > lastIndex) {
        const dimmedText = processed.substring(lastIndex, match.start);
        result.push(`<span style="opacity: 0.5">${dimmedText}</span>`);
      }
      // Add bright text for the match
      result.push(processed.substring(match.start, match.end));
      lastIndex = match.end;
    }

    // Add dimmed text after last match
    if (lastIndex < processed.length) {
      const dimmedText = processed.substring(lastIndex);
      result.push(`<span style="opacity: 0.5">${dimmedText}</span>`);
    }

    // If no matches found, dim everything
    if (mergedMatches.length === 0) {
      return `<span style="opacity: 0.5">${processed}</span>`;
    }
...
const heroFeatureSettings = computed(() => {
  ...
  const activeFeatures = features.value
    .filter(
      (f) =>
        f.tag.startsWith("ss") || f.tag.startsWith("cv") || f.tag === "dlig"
    )
    .slice(0, 9)
    .map((f) => `'${f.tag}' 1`)
    .join(",");
